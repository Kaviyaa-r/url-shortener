...
const getStrength = (pw) => {
  if (!pw) return { score: 0, text: '', color: '#1e293b', width: '0%' };
  let score = 0;
  ...
  if (score <= 1) return { score: 1, text: 'Weak', color: '#ef4444', width: '25%' };
  if (score <= 2) return { score: 2, text: 'Fair', color: '#f59e0b', width: '50%' };
  if (score <= 3) return { score: 3, text: 'Good', color: '#3b82f6', width: '75%' };
  return { score: 4, text: 'Strong', color: '#10b981', width: '100%' };
};
...
const Signup = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    else if (name.trim().length < 2) { setNameError('Name must be at least 2 characters'); isValid = false; }
    ...
    <div style={bg}>
      ...
      <div style={glassCard}>
        <div style={{ textAlign: 'center', marginBottom: '32px' }}>
          ...
          <p style={{ color: '#64748b', fontSize: '15px' }}>Create your account</p>
        </div>

        <form onSubmit={handleSubmit}>
          {/* Name */}
          <div style={{ marginBottom: '16px' }}>
            <label style={{ display: 'block', color: '#94a3b8', fontSize: '13px', fontWeight: 600, marginBottom: '8px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Full Name</label>
            <input type="text" value={name} onChange={e => setName(e.target.value)}
              onFocus={() => setFocusedField('name')} onBlur={() => setFocusedField(null)}
              style={inputStyle('name')} placeholder="John Doe" />
            {nameError && <p style={{ color: '#ef4444', fontSize: '12px', marginTop: '6px' }}>{nameError}</p>}
          </div>
          ...
          <div style={{ marginBottom: '24px' }}>
            ...
            <div style={{ position: 'relative' }}>
              <input type={showPass ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)}
                ...
                style={{...inputStyle('password'), paddingRight: '44px'}} placeholder="••••••••" />
              <button
                type="button"
            ...
            {password && (
              <div style={{ marginTop: '10px' }}>
                <div style={{ height: '4px', background: 'rgba(255,255,255,0.06)', borderRadius: '99px', overflow: 'hidden' }}>
                  <div style={{ height: '100%', width: strength.width, background: strength.color, borderRadius: '99px', transition: 'all 0.4s ease' }} />
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '4px' }}>
                  <span style={{ fontSize: '11px', color: '#475569' }}>Password strength</span>
          ...
          <button type="submit" disabled={loading} className="btn-interactive" style={{ ...primaryBtn, opacity: loading ? 0.7 : 1 }}>
